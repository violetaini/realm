...
## v3.2.8 更新重点 (violetaini fork)

- **彻底移除 Web 可视化面板**：永久关闭并彻底剔除所有面板相关代码与服务，消除开放 HTTP 端口遭受全网扫描与渗透的潜在安全风险，保持纯净轻量 CLI。
- **严重缺陷修复**：彻底修复在 SSH 终端异常断开、关闭窗口或休眠时，`read` 读到 EOF 导致脚本陷入 `while true` 疯狂空转、长久占用 CPU 的 Bug。
- **性能优化（智能自适应）**：安装时自动检测系统物理内存（MemTotal），智能分级配置零拷贝管道容量（`< 768M` 维持默认 64KB 防 OOM，`1G ~ 3G` 启用 128KB `-p 32`，`>= 4G` 启用 256KB `-p 64`）。
- 自动更新源切换至本维护仓库。
...
curl -L https://raw.githubusercontent.com/violetaini/realm/main/realm.sh -o realm.sh
chmod +x realm.sh
bash ./realm.sh
```
